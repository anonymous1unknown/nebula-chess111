...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      // א. קריאה ל-login וקבלת הטוקנים
      const { data: tokens } = await authApi.login(
        form.username_or_email,
        form.password,
      )

      // ב. שמור את הטוקנים ב-store לפני כל קריאה אחרת —
      //    כך ה-Request Interceptor ב-client.ts ימצא אותם
      //    ויצרף את ה-Authorization header ל-usersApi.me()
      useAuthStore.getState().setTokens(tokens)

      // ג. עכשיו אפשר לקרוא ל-/users/me בבטחה — הטוקן כבר ב-store
      const { data: user } = await usersApi.me()

      // ד. שמור הכל יחד — user + tokens — state מלא וסופי
  ...
  return (
    <AuthLayout title="Welcome back" subtitle="Sign in to your account">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="text-sm text-nebula-300 font-medium mb-1.5 block">
            Username or Email
          </label>
          <input
            className="input"
        ...
        <div>
          ...
          <div className="relative">
            ...
            <button
              type="button"
              className="absolute right-3 top-1/2 -translate-y-1/2 text-nebula-400 hover:text-nebula-200"
        ...
        <p className="text-center text-sm text-nebula-400">
          ...
          <Link
            to="/register"
            className="text-nebula-300 hover:text-white font-semibold transition-colors"
          >
            Create one
          </Link>
        </p>
