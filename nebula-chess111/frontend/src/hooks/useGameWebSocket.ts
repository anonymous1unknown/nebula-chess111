...
export function useGameWebSocket(gameId: string | undefined) {
  ...
  const handleMessage = (msg: WsMessage) => {
    switch (msg.type) {
      ...
      // FIX: handle game_started — published by the REST /join endpoint when
      // the second player joins a PvP game. The waiting player (creator) is
      // already connected on WS and needs this to know the game has begun.
      case 'game_started': {
        const d = msg.data as GameState
        setState(d)
        toast.success('Your opponent joined — game starting!', { duration: 3000 })
        break
      }
      ...
      case 'game_over': {
        ...
        const text = d.resigned_by
          ? `${d.resigned_by} resigned`
          : `Game over — ${d.result?.replace(/_/g, ' ')}`
      ...
      case 'player_disconnected': {
        ...
        addChat({
          username: 'System',
          message:  `${d.username} disconnected`,
          ts:       new Date().toISOString(),
          system:   true,
        })
        break
